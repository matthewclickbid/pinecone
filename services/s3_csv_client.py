...
class S3CSVClient:
    def __init__(self, bucket_name: Optional[str] = None, chunk_size: int = 1000):
        """
        Initialize S3 CSV client for reading large CSV files.
        ...
        logger.info(f"S3CSVClient initialized with bucket: {self.bucket_name}, chunk_size: {chunk_size}")
    
    def fetch_csv_data(self, s3_key: str, start_date: str, end_date: str, 
                      date_column: str = 'created_at') -> Iterator[List[Dict[str, Any]]]:
        """
        Fetch and filter CSV data from S3 in chunks for memory efficiency.
